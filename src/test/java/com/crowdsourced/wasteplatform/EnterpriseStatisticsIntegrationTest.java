...
class EnterpriseStatisticsIntegrationTest {
    ...
    @Test
    void overviewMonth_shouldIncreaseAfterNewCollectedReport() throws Exception {
        JsonNode before = getEnterpriseOverview();
        long beforeReceived = before.get("totalReportsReceived").asLong();
        long beforeCompleted = before.get("totalCompletedReports").asLong();
        BigDecimal beforeWeight = before.get("totalRecycledWeightKg").decimalValue();
        ...
        JsonNode after = getEnterpriseOverview();
        assertThat(after.get("totalReportsReceived").asLong()).isGreaterThanOrEqualTo(beforeReceived + 1);
        assertThat(after.get("totalCompletedReports").asLong()).isGreaterThanOrEqualTo(beforeCompleted + 1);
        assertThat(after.get("totalRecycledWeightKg").decimalValue()).isGreaterThanOrEqualTo(beforeWeight.add(new BigDecimal("2.500")));
    }

    private JsonNode getEnterpriseOverview() throws Exception {
        MvcResult result = mockMvc.perform(get("/enterprise/statistics/overview?range=MONTH")
    ...
    static class CreatePayload {
        ...
        public String description = "Enterprise statistics test";
        ...
        public String addressText = "Stats enterprise";
        public List<String> imageUrls = List.of("https://example.com/stats-enterprise.jpg");
    }
}
