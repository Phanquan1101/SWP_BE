...
class CollectorStatisticsIntegrationTest {
    ...
    @Test
    void collectorOverview_shouldIncreaseAssignedCompletedAndProofs() throws Exception {
        ...
        uploadProof(assignmentId);

        JsonNode after = getCollectorOverview();
        long assigned = after.get("totalAssignedRequests").asLong();
        long completed = after.get("totalCompletedRequests").asLong();
        long proofs = after.get("totalProofsUploaded").asLong();

        assertThat(assigned).isGreaterThanOrEqualTo(1);
        assertThat(completed).isGreaterThanOrEqualTo(1);
        assertThat(proofs).isGreaterThanOrEqualTo(1);

        double expectedRate = assigned == 0 ? 0.0
            : BigDecimal.valueOf(completed)
                .divide(BigDecimal.valueOf(assigned), 4, RoundingMode.HALF_UP)
                .setScale(2, RoundingMode.HALF_UP)
                .doubleValue();
        assertThat(after.get("completionRate").asDouble()).isEqualTo(expectedRate);
    }

    private JsonNode getCollectorOverview() throws Exception {
        MvcResult result = mockMvc.perform(get("/collector/statistics/overview?range=MONTH")
    ...
    private void uploadProof(String assignmentId) throws Exception {
        String payload = """
            {
              "proofUrls": [
                "https://example.com/proof-stats-collector.jpg"
              ],
              "takenAt": "2026-03-15T10:00:00Z"
            }
            """;
        mockMvc.perform(post("/collector/assignments/" + assignmentId + "/proof")
                .header("Authorization", "Bearer " + collectorToken)
                .contentType(MediaType.APPLICATION_JSON)
                .content(payload))
            .andExpect(status().isOk());
    ...
    static class CreatePayload {
        ...
        public String description = "Collector statistics test";
        ...
        public String addressText = "Stats collector";
        public List<String> imageUrls = List.of("https://example.com/stats-collector.jpg");
    }
}
